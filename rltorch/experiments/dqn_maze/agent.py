...
class DQNMazeAgent:
    ...
    def __init__(self, **kwargs):
        ...
        self.use_one_hot = kwargs["use_one_hot"]
        if self.use_one_hot:
            self.obs_dim = (self.maze_size*2,)
        else:
        ...
        print(self.dqn)
        self.use_target_network = kwargs["use_target_network"]
    ...
    def process_obs(self, o):
        if not self.use_one_hot:
            return o
        o = o.reshape(-1)
        one_hot = np.zeros(self.obs_dim)
        for d, i in enumerate(o):
            one_hot[d*self.maze_size + int(i)] = 1.0
        return one_hot
    ...
    def optimize(self):
        ...
        with torch.no_grad():
            q_next_vals_raw = self.dqn(s_batch)
            target_next_q_val_raw = self.dqn(next_s_batch)

        for i in range(min(3, self.batch_size)):
            print(f"\ns: {s_batch[i]}")
            print(f"a: {a_batch[i]}")
            print(f"s': {next_s_batch[i]}")
            print(f"Q_t(s, .) = {q_vals_raw[i]}")
            print(f"Q_t(s', .) = {target_q_val_raw[i]}")
            print(f"TD_error = {target[i]-q_vals[i]}")
            print(f"Q_t+1(s, .) = {q_next_vals_raw[i]}")
            print(f"Q_t+1(s', .) = {target_next_q_val_raw[i]}")
            print()
    ...
    def run_episode(self):
        ...
        while not done and self.steps_done < self.training_steps:
            ...
            next_o = self.process_obs(next_o)
            ...
            if self.use_target_network and \
               self.steps_done % self.target_update_freq == 0:
                self.update_target_net()
